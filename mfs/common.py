				...
				for cat_name, cat_contents in src_cats.items():
					...
					for pkg_block in cat_contents["packages"]:
						...
						if "url" in pkginfo and isinstance(pkginfo["url"], str):
							sources = [pkginfo["url"]]
							del pkginfo["url"]
						elif "urls" in pkginfo and isinstance(pkginfo["urls"], list):
							sources = pkginfo["urls"]
							del pkginfo["urls"]
						elif "package" in pkginfo:
							sources = []
						else:
							raise ValueError(f"Expecting string ('url') or list of strings ('urls'): {pkginfo}")
						...
						if "srcdir" in pkginfo:
							pkginfo["srcdir"] = pkginfo["srcdir"].format(
								version=pkginfo["version"]
							)
		...
		if "," not in sources and "package" in self.sources[sources]:
			...
			if not os.path.exists(package_path):
				raise FileNotFoundError(
					f"Package referenced for {sources} not found: {package_path}"
				)

			out = (
				f"cd ${{CLFS}} && "
				f"tar xpvf ${{CLFS}}/packages/{package_name} "
				f"-C ${{CLFS}} || true\n"
			)

			out += (
				f"export "
				f"{sources.replace('-', '_').upper()}_VERSION="
				f"\"{self.sources[sources]['version']}\"\n"
			)
			...
			for source in sources.split(','):
				...
				for in_url in self.sources[source]["sources"]:
					...
					out += (
						f"export "
						f"{source.replace('-', '_').upper()}_VERSION="
						f"\"{self.sources[source]['version']}\"\n"
					)
